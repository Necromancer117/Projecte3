                    ...
                    <div class="flex">
                        ...
                        <div id="cratorShowsDropdwon" hidden class="absolute right-50 z-10 mt-2 origin-top-right rounded-md bg-white py-1 shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none">
                            <ul class="py-1 h-full text-sm text-gray-700" aria-labelledby="dropdownDefault">
                                ...
                                ?>
                                    <li>
                                        <a value="<?= date("Y", strtotime($showEditon[$i]["dia_inicio_edicion"])) ?>" data-role="year" data-id="<?= date("Y", strtotime($showEditon[$i]["dia_inicio_edicion"])) ?>" class="block py-2 px-4 hover:bg-gray-100"><?= date("Y", strtotime($showEditon[$i]["dia_inicio_edicion"])) ?></a>
                                    </li>
                                <?php
                                }
                                ?>
                                <li>
                                    <a data-role="addYear" class="block py-2 px-4 bg-gray-50 hover:bg-gray-100">Add Year</a>
                                </li>
                            </ul>
                        </div>
                        <a id="addShow" data-role="addShow" class="cursor-pointer bg-indigo-600 text-white block px-3 py-2 rounded-md text-base font-medium">Add show</a>
                    </div>
                </div>
            </div>
                    ...
                    <tbody>
                        ...
                        for ($i = 0; $i < COUNT($showList); $i++) {
                            ...
                            <?php
                                ...
                                <tr id="<?= $showList[$i]["id_espectaculo"] ?>" class="bg-white border-t">
                                    <td scope="row" class="py-4 px-6 font-medium text-gray-900 whitespace-nowrap"><?= $showList[$i]["id_espectaculo"] ?></td>
                                    <td data-target="title" scope="row" class="py-4 px-6 font-medium text-gray-600 whitespace-nowrap"><?= $showList[$i]["nombre_espectaculo"] ?></td>
                                    <td data-target="type" scope="row" class="py-4 px-6 font-medium text-gray-600 whitespace-nowrap"><?= $showList[$i]["tipo_espectaculo"] ?></td>
                                    <td data-target="description" hidden><?= $showList[$i]["descripcion_espectaculo"] ?>"</td>
                ...
                <div class="fixed inset-0 z-50 flex justify-center items-center bg-black bg-opacity-50">
                    <div class="bg-white rounded shadow-lg p-8 relative w-1/2">
                        <div class="w-full">
                            ...
                            <!--URL AND QR-->
                            <div id="qrContainer" class="flex flex-wrap -mx-3 mb-6">
                                <div class="w-full px-3 mb-6 md:mb-0">
                                    <div class="flex">
                            ...
                            <div class="flex justify-between">
                                ...
                                <a data-role="applyChanges" id="applyChanges" class="cursor-pointer bg-indigo-600 text-white block px-3 py-2 rounded-md text-base font-medium">Apply changes</a>
                                <a data-role="discard" class=" hidden cursor-pointer border border-indigo-600 text-indigo-600 block px-3 py-2 rounded-md text-base font-medium">Discard</a>
                                <a data-role="add" id="add" class="hidden cursor-pointer bg-indigo-600 text-white block px-3 py-2 rounded-md text-base font-medium">Add</a>
                            </div>
                        </div>
                    </div>
                </div>
